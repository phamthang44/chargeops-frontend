...
import { Button, FormField, IconInfo, IconShield, Modal } from '@chargeops/ui';
import { formatVnd, type StationApprovalSummary, type Station } from '@chargeops/api';
...
export function IssueLicenseModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(fee) || fee < 0) {
      setError(t('issueLicenseModal.errFeePositive', { defaultValue: 'Phí giấy phép phải là số không âm.' }));
      return;
    }
    setError(null);
    ...
    <Modal open={open} onClose={handleClose} maxWidth={480}>
      <form onSubmit={handleSubmit}>
        ...
        <div className="mt-3.5">
          <FormField
            label={t('issueLicenseModal.feeLabel', { defaultValue: 'Phí License đã ghi nhận (VND)' })}
            required
